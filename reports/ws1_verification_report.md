...
### Defect 2 — growth-step hang at cycle 500 (WORKED AROUND)
With `DECADIC_GROWTH_ENABLED=1`, the growth evaluation (`neural_pipeline.py` ~line 420, every `growth_interval=500` cycles) hangs the cycle loop with no exception logged — suspected CUDA allocation stall in `grow_step`/`reset_optimizer_state` under 10GB VRAM pressure at the `full` preset. Reproduced exactly at cycle 501. **Workaround:** growth disabled for verification. **Needs:** guarded/async growth step with a timeout, or VRAM headroom check before growing.
...
1. ~~Debug defect 3~~ DONE — fixed and verified (10,984 cycles / 20 min, zero stalls).
2. Flip the default overload policy to `drop_oldest` (now safe post-tombstone-fix); update the two tests that pin `"block"` (`test_parallel_sessions.py:100`, `test_stage_pipeline.py:28`).
3. Hunt the growth hang: `scripts\stall_hunt.ps1 -Growth` reproduces it with the same dump capture; then guard/async-offload the growth step before re-enabling structural growth.
4. Commit this work, then proceed to WS2 (measurement harness) and its 12-hour soak per `docs/verification_measurement_attention_gate_plan.md`.
