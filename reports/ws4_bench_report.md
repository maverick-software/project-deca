...
### WS4B M3.4 — multi-row UNWIND inserts (2026-07-05, LANDED)

Both decay drivers from the soak are now fixed at the source.

**M3.4 (flusher, SECONDARY driver).** `_group_stmts` coalesces consecutive runs of the same statement template into ONE `UNWIND $rows AS r …` statement (`decadic/memory/kuzu_graph.py`). Runs only — global statement order is preserved exactly, so create-before-reference and delete/recreate orderings are untouched; mixed batches degrade gracefully toward the old shape. The failure path is unchanged (replays from OPS, statement-shape agnostic), so grouping never weakens durability. Verified on kuzu 0.11.3: list-of-dict params bind as structs; NULLs, `FLOAT[16]` vectors, and heterogeneous int/float/None rows all round-trip; `MATCH`-inside-`UNWIND` works for SET and edge-CREATE. New telemetry `graph_flush_rows` / `graph_flush_stmts` (the compression ratio) surfaces in `persistence_metrics` and the diag summary. Flag: `DECADIC_KUZU_MULTIROW=1` (default; `0` = per-statement parity arm).

**Growth governance (PRIMARY driver — graduated from ledger to fix).** Growth is now EARNED, not scheduled (`decadic/cycle/neural_pipeline.py`). Two gates over the old absolute-threshold trigger: (a) *progress* — learning must have STALLED at current capacity (relative pc-EMA improvement over the last interval below `DECADIC_GROWTH_MIN_PROGRESS`, default 0.01); (b) *efficacy* — the PREVIOUS growth event must have PAID (EMA improved ≥ `DECADIC_GROWTH_MIN_GAIN`, default 0.02, since it fired) or growth is refused. First eligibility check never grows blind (no progress baseline). `growth_blocked_reason` + `growth_pc_ema_at_last_growth` are logged. Both gates disabled by setting their config to 0 (the forced-growth escape hatch the mechanics fixtures use). On the soak scenario (flat-high loss) this converts *8 events/hour* → *exactly 1 honest try, then blocked* — the intended lifelong-run behavior.

Tests: `test_growth_governance.py` (7 scenarios incl. the soak curve, re-arm, checkpoint persistence) + 4 new `test_ws4b_m34_*` (pure grouping, node compression+durability+parity, EDGE create/set durability, grouped-batch failure replay). Growth-mechanics fixtures pin the governance escape hatch.

**Post-soak fix (edge-SET alias collision).** The first watched re-run surfaced a bug the node-only tests missed: the UNWIND alias was `r`, which shadowed the relationship variable in the edge templates (`MATCH (a)-[r:RELATES]->(b) … SET r.*`) → kuzu `Binder exception: r has data type STRUCT … but REL was expected`. Every edge-containing batch fell to per-op replay (durability held via the replay path — the server never crashed — but M3.4's win was lost for edges and the log filled with errors). Fixed by moving the alias to `__u` (collides with none of `e`/`a`/`b`/`r`/`s`); verified all 8 templates against kuzu 0.11.3 and pinned by `test_ws4b_m34_multirow_edges_create_and_set`.

**Body reconnect resilience.** The same run also exposed that a transient socket reset (Windows `WinError 64`) killed the whole soak seconds in — the body adapter had no reconnect. `main()` now wraps the run in a reconnect loop that re-binds to the SAME server-side agent (brain/memory/growth persist across the blip), with backoff and a give-up cap (`DECADIC_BODY_MAX_RECONNECTS`, default 1000).

Remaining WS4B: **M3.5** — re-run the 1-h soak on the fixed stack + a growth-frozen control arm (`DECADIC_GROWTH_ENABLED=0`) to confirm the curve plateaus and quantify each fix's share.

Ledger (still open, longer-horizon): **growth governance, phase 2** — metabolic capacity budget and pruning/consolidation as the active counter-force (phase 1 above only *gates* growth; it does not yet reclaim capacity).
