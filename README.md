...
**One click (Windows):** run `scripts/launch_decadic.ps1` — it starts the server and
the web UI in their own windows and opens the dashboard in your browser. To put it on
the desktop, create a shortcut to:

```text
powershell.exe -ExecutionPolicy Bypass -NoProfile -File "<repo>\scripts\launch_decadic.ps1"
```
...
| `DECADIC_SELF_MODEL_FEEDBACK` | Self-state feedback spine (self-model program): the previous cycle's self-report (A state-of-mind ‖ C narrative ‖ E metacognition) is injected back into the stack so internal state shapes the next cycle. Default `0` off; zero-init projection ⇒ on is byte-identical until it learns. Rebuilds the brain on toggle. |
| `DECADIC_GWT_ENABLED` | Real global workspace (self-model program): replaces the working-memory EMA blend into A with a capacity-limited winner-take-all competition + ignition threshold + broadcast (to A, the self-model spine, the episodic salience, and the narrative). Default `0` off (EMA off-branch is byte-identical). Live toggle (no rebuild). Tuned by `DECADIC_GWT_IGNITION_THRESHOLD` (default `0.5` share of salience mass), `DECADIC_GWT_CAPACITY` (coalition size, default `1`), `DECADIC_GWT_TEMPERATURE` (default `1.0`), `DECADIC_GWT_SALIENCE_BOOST` (episodic-salience lift, default `1.0`). |
| `DECADIC_INTEGRATION_WINDOW_MS` | Explicit temporal-integration window (self-model program): bind a span of bottom-up percepts into one committed "now". The agent acts on the last committed moment until the window (this many wall-clock ms, or `DECADIC_INTEGRATION_WINDOW_MAX_FRAMES` cycles, default `8`) closes and a new now is bound — so longer windows shift when perception updates. Default `0` off (the freshest percept is always now; byte-identical). Live setting (no rebuild). |
| `DECADIC_PREDICTIVE_AFFECT` | Predictive affect (self-model program): a small forward model predicts the next-step affective context (viability/pain/pleasure/priority) from the previous cycle's actual affect, and the predicted delta colours the episodic proxy before it is projected into the stack — so the agent perceives in light of how it expects to feel. Default `0` off; zero-init output ⇒ on is byte-identical until it learns (it rides the main prediction-error graph). Scaled by `DECADIC_PREDICTIVE_AFFECT_GAIN` (default `1.0`). Rebuilds the brain on toggle. |
| `DECADIC_REPRESENTED_SELF` | Represented self (self-model program): the agent's interoception (reservoirs), affect, and capability (its discovered body schema) are written as content onto the egocentric self-node, "controls" edges bind the self to its learned body parts, and a compact self-node embedding is fed back through a dedicated zero-init spine ingress — so the self becomes a represented object the agent models. Default `0` off; zero-init ingress ⇒ on is byte-identical until it learns. Rebuilds the brain on toggle. |
| `DECADIC_MEMORY_EFFICIENT_TRAINING` | Memory-efficient training path (self-model program, hardware-gated): the per-cycle train step uses 8-bit Adam (via `bitsandbytes`) + a bf16 autocast forward **on CUDA**, cutting optimizer-moment + activation memory so the heavy 250m/500m/1b presets can fit a single consumer GPU. Default `0` off; falls back silently to fp32 Adam (no `bitsandbytes` / CPU) so the standard path is byte-identical. See `scripts/integration_sweep.py` for the integration (PCI) falsification harness. |
