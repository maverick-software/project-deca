...
| G9 | **Hearing is event-driven and client-trusted** | Audio exists only when a client chooses to attach `obs.audio`; between such observations the agent is deaf; no path from a system microphone at all | An always-on **auditory intake organ** server-side: continuous capture (sim bus and/or system mic) into a ring buffer; every observation automatically carries the freshest chunk — hearing becomes a property of the body, not of the mail |
...
- **3.7 Acoustic rigs — how voice becomes sound (settled).** For the agent, sound IS the waveform in `obs.audio`; whether it ever moves air is as irrelevant to its cognition as MuJoCo's photons never striking a retina. Two rigs, one contract:
  - **Rig 1 — simulated acoustics (default; ALL training).** No OS audio, no files, no speaker. MuJoCo has no audio engine, so the world owns a **mixing bus**: the formant synth (pure numpy DSP) renders `voice_u` to ~1,600 samples/cycle (16 kHz × ~100 ms), interpolated against the previous cycle's params (no frame-boundary clicks); the bus sums the agent's waveform + scheduled caregiver utterances with distance attenuation and writes the mix into the NEXT observation's `obs.audio` as the same base64 wav chunk the Whisper path decodes today. The loop closes entirely in arrays between processes — a thousand hours of babble costs nothing and wakes no one. An optional **monitor tee** streams the bus to the operator's speakers (for us, not for it).
  - **Rig 2 — desk rig (live interaction/demos).** Streams, never files: a callback-driven output stream (`sounddevice`-class API) drains the synth through a ring buffer to a physical speaker (10–30 ms buffers, well inside the ~100 ms cycle); an input stream chunks the microphone (~100–200 ms, cycle-aligned) into `obs.audio`. The loop closes through actual air — the agent's voice and the operator's voice arrive mixed by the room, like a child hearing itself and its mother at once. Self-masking is efference-aware (the agent knows what it just said; hearing discounts it), mirroring the biological solution.
  - **Rig-indifference principle:** the agent must not be able to distinguish the rigs — identical `obs.audio` contract, identical proprioception. This is enforced, not hoped: the M2 loopback smoke runs the SAME probe assertions on both rigs. Train in Rig 1; meet Charles in Rig 2.
- **3.8 Continuous auditory intake (G9, settled).** Capture is continuous; consumption stays cycle-quantized — exactly the biological arrangement (the cochlea never sleeps; the cycle samples it, and the integration window binds a span into one "now"). An `AudioIntake` service runs server-side as a background worker: a callback-driven input stream (system microphone via `sounddevice`, and/or the Rig-1 mixing bus tap) fills a rolling ring buffer; at each observation ingest the runtime attaches everything-since-last-read (capped ~250 ms, drop-oldest on overrun — the perception pipeline's own overload semantics) to `obs.audio`. **Precedence: client-supplied audio wins; the intake fills silence** — recorded scenarios stay byte-reproducible while live rooms become audible. `DECADIC_AUDIO_INTAKE = off | mic | bus` (default off; parity preserved). Two cost guards: an energy-threshold **silence gate** skips Whisper encoding on quiet chunks (the ambient room must not tax the GPU), and continuous-intake encode cost is measured before any default moves. Hearing is deliberately DECOUPLED from speaking: this lands in M0 so the agent can hear the room — and Charles — long before it can answer.
